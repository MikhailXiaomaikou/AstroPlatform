...
- [`src/api/client.ts`](./frontend/src/api/client.ts) — Axios + typed SSE streaming. `ThinkingEvent` union covers `agent_text` (with `draft` / `redacted_count`, surfaced in the timeline as a "draft · unverified" label plus an "N value(s) withheld" note) / `tool_call` / `tool_result` / `status` / **`honest_abstention`** / `error`. `getAIBackendStatus()` feeds the F4 pre-send gate.
...
7. SSE events: `text` (final reply), `agent_text` (live thinking), `tool_call`, `tool_result` (`live: true` during loop + final consolidated), `status` (heartbeats), `gate_event`, `honest_abstention`, `error`, `done`.
8. **Streamed-draft contract** (2026-09-02 review H5, corrected 2026-09-03). Everything the loop streams leaves the process *before* step 6's gates run, so two frames are constrained:
   - `agent_text` carries **intermediate turns only** — a turn that also calls a tool. The final, answer-carrying turn is no longer streamed here; it reaches the client through the gated `text` frame. Each event ships `draft: true`, `not_claimable: true`, and `redacted_count: <int>`. Before emission the content passes through `honesty.redact_gated_values`, which decides each number token *at its own position* by three rules and replaces only the flagged spans with `[withheld]`; `redacted_count` is how many were replaced. The rules are: (1) the echo gate's rule — an exact match to a rejected user-pasted value, little-h tokens included; (2) the withheld-posterior gate's rule — within 1% of a non-publication statistic, with the interval-idiom and little-h exemptions applied per token; (3) **the final validator itself** — `claim_validator.validate_claims(text, tool_results)`, the same call with the same defaults that step 6 makes on the final reply, and the value of every claim it reports uncited is withheld. Rule 3 has no grammar of its own: no label list, bridge list, tolerance or per-parameter bucket lives in the redactor (an earlier revision kept one, and every review round found a spelling the validator knew and it did not). Whatever the final gate would refuse — an invented `H0`, an echoed model input, TeX `\pm`, an `H_{0}`/`Ωₘ` alias, an age in Gyr, a requested redshift no tool echoed — the draft withholds, and whatever the validator grounds, the draft keeps. Numbers the validator does not treat as claims — years, arXiv ids, counts — survive byte-for-byte. On the deterministic cosmology/research route the model's prose is dropped entirely and replaced by a fixed "Draft intermediate prose withheld" placeholder, so those turns stream no model text at all.
   - `gate_event` (one per gate intervention; built by `app/observability/gate_events.py`) carries a **redacted preview**. The record a gate builds quotes the pre-gate draft — exactly the text just refused — so the two sinks deliberately differ: the local, gitignored JSONL triage sink (`settings.gate_events_jsonl_path`, default `data/gate_events.jsonl`) keeps the draft in full because diagnosing a false kill requires reading the observed evidence verbatim, while the copy handed to the SSE stream goes through the same redactor first (`gate_events.redact_event_for_wire`), covering `draft_preview`, `final_preview` and the string/numeric leaves of `details`. It fails closed: a redactor that raises drops the prose rather than shipping it.
   - **Not covered:** `tool_call.input` is still model-authored text streamed verbatim. It is a different channel with a different design question (tool arguments are meant to be readable) and is tracked in `plan/cosmology-completion-backlog.md` §P3, not fixed here.
9. Auto-save after each turn; auto-title from first user message; F4 pre-send gate blocks `Send` when no AI backend is configured.
