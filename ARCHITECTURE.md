...
8. **Streamed-draft contract** (2026-09-02 review H5, corrected 2026-09-03). Everything the loop streams leaves the process *before* step 6's gates run, so two frames are constrained:
   - `agent_text` carries **intermediate turns only** — a turn that also calls a tool. The final, answer-carrying turn is no longer streamed here; it reaches the client through the gated `text` frame. Each event ships `draft: true`, `not_claimable: true`, and `redacted_count: <int>`. Before emission the content passes through `honesty.redact_gated_values`, which asks the two output gates (`untrusted_evidence_echo_values`, `nonpublication_posterior_values`) for their hits *on that exact text* and replaces only those spans with `[withheld]`; `redacted_count` is how many were replaced. Numbers the gates do not flag — years, arXiv ids, requested redshifts — survive byte-for-byte. On the deterministic cosmology/research route the model's prose is dropped entirely and replaced by a fixed "Draft intermediate prose withheld" placeholder, so those turns stream no model text at all.
