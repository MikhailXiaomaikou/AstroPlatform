...
- `prompts/` — Three-layer prompt tree (M1 Phase 4): `base.md` + `core/*.md` (cross-cutting rules) + `modules/<name>/{manifest.yaml, prompt.md, appendix.md}`. **1 active module** (`cosmology`) + **12 dormant modules** (`_dormant_agn`, `_dormant_galaxy_morphology`, `_dormant_high_z_galaxy`, `_dormant_image_reduction`, `_dormant_paper_export`, `_dormant_paper_tool_mining`, `_dormant_pipeline_dag`, `_dormant_pulsar_timing`, `_dormant_radio`, `_dormant_stellar`, `_dormant_team_workspace`, `_dormant_xray_spectroscopy`). Don't delete dormant code — flipping `status` in the manifest is the promotion path. The `solar_system` and `exoplanet` active modules were extracted to the sibling standard-astro-verticals repo on 2026-06-03.
