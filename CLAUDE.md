...
- `services/` — 30+ service modules: ai_tools (**68 tools**, including `extract_literature_tables` and `fit_line_lfr` for cited literature-table line-relation fits; `list_cosmology_datasets`, `build_cosmology_likelihood`, and `build_cosmology_robustness_matrix` for observational-cosmology likelihood planning), astro_analysis, spectral_analysis_pro, photo_z_pro, bayesian_inference, **cosmology_mcmc** (typed distance-modulus emcee/Cobaya interface with publication diagnostics; inline rows audit-only, cached rows citeable), **cosmology_likelihoods** (dataset registry + Cobaya/CosmoSIS config builder), time_domain_pro, image_processing_pro, parsec_fetcher, transient_classifier, literature_engine, memory_service, code_executor, **claim_validator** (zero-fabrication numeric gate + provenance-v2 citation validator), **result_provenance** (EMPTY/FAILED/UNAVAILABLE/SYNTHETIC banners + reproducibility envelope + nested `provenance` object), **provenance_v2** (fallback registry, field-level schema/extractor, resolver helpers), provenance (versioned environment manifest), dossier_generator, vo_services, **connector_cache** (content-addressed, Null/SQLite/Redis, singleflight), **workflow_checkpoint** (resumable multi-step AI workflows), **sandbox/subprocess_backend** (crash-isolated `multiprocessing` spawn + rlimit + killpg + F0 payload-completeness guard)
...
- `api/chat.py` — **SYSTEM_PROMPT is ~57 KB / ~14 K tokens with 46 sections**, including provenance-v2 citation hierarchy, `ZERO-FABRICATION CONTRACT`, `STRUCTURED ABSTENTION`, `ADQL aggregate-function semantics`, `Cluster / association analysis idioms`, and literature-table / line-relation workflow rules at the top; literature-cited workflows for 16+ object classes below. Also defines `_parse_abstention_tag` / `_classify_abstention_reason` / `_render_abstention_card` for the `<tools_returned_nothing/>` structured-abstention flow, deterministic literature-table extraction / `fit_line_lfr` follow-up for line-relation prompts, and `GET /api/chat/ai_backend_status` for the F4 pre-send Send-button gate
