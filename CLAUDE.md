...
- `connectors/` — **24 connector keys** in `registry.py`. Provenance-v2 currently activates 6 keys (`vizier`, `gaia`, `simbad`, `ned`, `2mass`, `alma`) and gates the other 18 as `UNAVAILABLE` before connector import. `2mass` is implemented in `twomass.py`; ALMA is active for Science Archive observation metadata only; SDSS direct SQL (`run_sdss_sql`) is still maintenance-gated until it has independent `archive_version` provenance.
...
- `services/` — **61 service modules** (see `scripts/stats.sh`): ai_tools (**87 tools**, including `extract_literature_tables` and `fit_line_lfr` for cited literature-table line-relation fits; `list_cosmology_datasets`, `build_cosmology_likelihood`, and `build_cosmology_robustness_matrix` for observational-cosmology likelihood planning), astro_analysis, spectral_analysis_pro, photo_z_pro, bayesian_inference, **cosmology_mcmc** (typed distance-modulus emcee/Cobaya interface with publication diagnostics; inline rows audit-only, cached rows citeable), **cosmology_likelihoods** (dataset registry + Cobaya/CosmoSIS config builder), time_domain_pro, image_processing_pro, parsec_fetcher, transient_classifier, literature_engine, memory_service, code_executor, **claim_validator** (zero-fabrication numeric gate + provenance-v2 citation validator), **result_provenance** (EMPTY/FAILED/UNAVAILABLE/SYNTHETIC banners + reproducibility envelope + nested `provenance` object), **provenance_v2** (fallback registry, field-level schema/extractor, resolver helpers), provenance (versioned environment manifest), dossier_generator, vo_services, **connector_cache** (content-addressed, Null/SQLite/Redis, singleflight), **workflow_checkpoint** (resumable multi-step AI workflows), **sandbox/subprocess_backend** (crash-isolated `multiprocessing` spawn + rlimit + killpg + F0 payload-completeness guard)
...
- `api/chat.py` — **SYSTEM_PROMPT is ~108 KB / ~27 K tokens with 59 sections** (see `scripts/stats.sh`), including provenance-v2 citation hierarchy, `ZERO-FABRICATION CONTRACT`, `STRUCTURED ABSTENTION`, `ADQL aggregate-function semantics`, `Cluster / association analysis idioms`, and literature-table / line-relation workflow rules at the top; literature-cited workflows for 16+ object classes below. Also defines `_parse_abstention_tag` / `_classify_abstention_reason` / `_render_abstention_card` for the `<tools_returned_nothing/>` structured-abstention flow, deterministic literature-table extraction / `fit_line_lfr` follow-up for line-relation prompts, and `GET /api/chat/ai_backend_status` for the F4 pre-send Send-button gate
...
- Active connector keys are exactly `vizier`, `gaia`, `simbad`, `ned`, `2mass`, and `alma`. The other 18 keys in `CONNECTORS_KEYS` return the `UNAVAILABLE` maintenance banner before connector import. ALMA v2 support is limited to Science Archive observation metadata; it does not provide derived line luminosity, line flux, or FWHM measurements.
