...
async def export_chat_as_markdown(req: ChatMarkdownRequest):
    ...
    lines.append(f"*Generated by Astro Platform AI Research Assistant*")

    md = "\n".join(lines)
    return StreamingResponse(
        iter([md]),
        media_type="text/markdown",
        headers={"Content-Disposition": f'attachment; filename="ai_research_chat.md"'},
    )
...
class LatexReportRequest(_BaseModel):
    ...
    messages: list[dict] = []
    title: str = "Astro Platform Research Report"
    abstract: str = ""
    author: str = "Astro Platform User"
...
async def export_report_latex(req: LatexReportRequest):
    ...
    lines: list[str] = []
    lines.append(r"% Generated by Astro Platform on " + date_str)
    ...
    lines.append(r"")
    if user_queries:
        lines.append(r"The following research queries were investigated using the Astro Platform AI research assistant:")
    ...
    for fi, fig_type in enumerate(figure_refs[:5]):
        ...
        lines.append(r"  \caption{" + _tex_escape(fig_type) + r" generated by Astro Platform.}")
    ...
    if unique_bibcodes:
        ...
        lines.append(r"")
        lines.append(r"% To use BibTeX, export the .bib file from Astro Platform")
...
async def export_report_bibtex(req: BibTeXRequest):
    ...
    bib_entries.append("% BibTeX entries extracted by Astro Platform")
    ...
    if not unique_bibcodes:
        ...
        for bib in unique_bibcodes:
            ...
            bib_entries.append(f"  author = {{}},")
            bib_entries.append(f"  title = {{}},")
            bib_entries.append(f"  journal = {{}},")
...
async def export_chat_as_notebook(req: ChatToNotebookRequest):
    ...
    cells.append({
        "cell_type": "markdown",
        "metadata": {},
        "source": [f"# {req.title}\n", "\nGenerated by Astro Platform AI Research Assistant\n"],
    })
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f'attachment; filename="ai_research_session.ipynb"'},
    )
