...
async def get_lineage(entity_id: str, user: User = Depends(get_current_user)):
    """Get full lineage graph for an entity."""
    from app.services.provenance import get_lineage as _get_lineage
    return _get_lineage(entity_id, owner_id=str(user.id))


@router.get("/{entity_id}/export/ivoa")
async def export_ivoa(entity_id: str, user: User = Depends(get_current_user)):
    """Export provenance as IVOA ProvDM XML."""
    from app.services.provenance import export_provenance_ivoa
    xml = export_provenance_ivoa(entity_id, owner_id=str(user.id))
...
async def doi_metadata(entity_id: str, title: str = "",
    ...
    return generate_doi_metadata(entity_id, title=title, owner_id=str(user.id))


@router.get("/{entity_id}/requirements.txt")
async def export_requirements(entity_id: str, user: User = Depends(get_current_user)):
    """Export pinned requirements.txt for reproducibility."""
    from app.services.provenance import export_requirements_pinned
    try:
        content = export_requirements_pinned(entity_id, owner_id=str(user.id))
...
async def reproduce(entity_id: str, user: User = Depends(get_current_user)):
    """Get reproducibility package."""
    from app.services.provenance import get_reproducibility_package
    return get_reproducibility_package(entity_id, owner_id=str(user.id))
