...
Module: **Radio Astronomy (NVSS / FIRST)**

## Activation procedure (M5+ or strategy change)

1. `mv _dormant_radio/ radio/`
2. In `manifest.yaml`: `status: dormant` → `status: active`
3. Add `radio` to the `ASTRO_RESEARCH_FOCUS` enum
4. In `chat.py`: add an if-branch in `load_active_modules()` for `focus == "radio"`
