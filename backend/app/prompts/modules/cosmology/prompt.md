...
| **"DESI DR2 evidence matrix"** / "compare DESI DR2 + Pantheon+, Union3, DES-SN5YR" / "DR2 tension lab" | `run_dark_energy_evidence_matrix(model="w0wa_cdm", ...)` — reads only byte-pinned official chains from an operator-provided mirror. These are published posterior samples, never reusable likelihoods. Shared DESI/CMB cells must return `correlated_tension_withheld`; never invent an independent-error tension sigma. |
