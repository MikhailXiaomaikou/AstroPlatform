...
os.environ.setdefault("ASTRO_RESEARCH_FOCUS", "solar_system")
...
def _check_env(provider: str) -> str | None:
    ...
    if focus != "solar_system":
        raise SystemExit(f"ASTRO_RESEARCH_FOCUS 必须是 'solar_system', 现在是 {focus!r}")
    if provider == "local":
...
async def main() -> None:
    ...
    print(f"工具数 (focus=solar_system): {len(_filter_tools_by_research_focus(TOOLS))}", flush=True)
