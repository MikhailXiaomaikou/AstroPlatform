...
> **版本说明（2026-08-07）**：上述 Standard Astro `1440/1440` 是确定性管道与自动审计规则的自检，模型没有在回路，不能作为模型行为增益的头条证据。模型真实参与的修复后自然措辞层为 `651/720`（90.4%，n=60），详见战役报告。评分文件生成后，代码又完成十二轮来源核验与路由加固；完整矩阵尚未在最新 `HEAD` 上重跑，所以本文数字是历史评测快照。
