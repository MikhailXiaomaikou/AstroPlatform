...
const translations: Record<string, Record<Lang, string>> = {
  ...
  "home.lead":     { en: "Chat your way through nine live archive connectors across three research modules — observational cosmology, the small-body solar system, and exoplanets. Ask for a likelihood chain, an HR diagram, an asteroid's NEATM diameter, or a transit fit — the assistant calls real archive APIs, records every seed and run id, and every number it cites traces back to the tool that produced it.",
                     zh: "通过一次对话调动 9 个在线档案连接器，横跨三个研究模块 —— 观测宇宙学、太阳系小天体、系外行星。让它跑似然链、绘制 HR 图、算小行星的 NEATM 直径或做凌星拟合 —— 助手调用的是真实的档案 API，每一个种子、每一个 run id 都记录在案，引用的每一个数字都能追溯到生成它的那次工具调用。",
                     fr: "Conversez avec neuf connecteurs d'archives actifs répartis sur trois modules de recherche — cosmologie observationnelle, petits corps du système solaire et exoplanètes. Demandez une chaîne de vraisemblance, un diagramme HR, le diamètre NEATM d'un astéroïde ou un ajustement de transit — l'assistant appelle les vraies API d'archives, enregistre chaque graine et chaque run id, et chaque chiffre qu'il cite remonte jusqu'à l'outil qui l'a produit.",
                     es: "Conversa con nueve conectores de archivos activos en tres módulos de investigación — cosmología observacional, cuerpos menores del sistema solar y exoplanetas. Pídele una cadena de verosimilitud, un diagrama HR, el diámetro NEATM de un asteroide o un ajuste de tránsito — el asistente llama a las APIs de archivo reales, registra cada semilla y cada run id, y cada número que cita se rastrea hasta la herramienta que lo produjo." },
