...
const translations: Record<string, Record<Lang, string>> = {
  ...
  "template.variable_desc":   { en: "Find a known Cepheid variable, obtain its light curve, detect the period using Lomb-Scargle, and perform phase-folding analysis", zh: "帮我找一颗已知的造父变星，获取其光变曲线数据，用 Lomb-Scargle 方法检测周期，并做相位折叠分析", fr: "Trouver une céphéide connue, obtenir sa courbe de lumière, détecter la période par Lomb-Scargle", es: "Encontrar una cefeida conocida, obtener su curva de luz, detectar el período con Lomb-Scargle" },
  ...
  "home.lead":     { en: "Chat your way through live archive connectors for observational cosmology. Ask for a DESI BAO likelihood chain, an HR diagram, an SN Ia distance fit, or a Planck-compressed CMB constraint — the assistant calls real archive APIs, records every seed and run id, and every number it cites traces back to the tool that produced it.",
                     zh: "通过一次对话调动面向观测宇宙学的在线档案连接器。让它运行 DESI BAO 似然链、绘制 HR 图、拟合 SN Ia 距离，或计算 Planck 压缩 CMB 约束 —— 助手调用的是真实的档案 API，每一个种子、每一个 run id 都记录在案，引用的每一个数字都能追溯到生成它的那次工具调用。",
                     fr: "Conversez avec des connecteurs d'archives en direct pour la cosmologie observationnelle. Demandez une chaîne de vraisemblance BAO DESI, un diagramme HR, un ajustement de distance SN Ia ou une contrainte CMB compressée de Planck — l'assistant appelle les vraies API d'archives, enregistre chaque graine et chaque run id, et chaque chiffre qu'il cite remonte jusqu'à l'outil qui l'a produit.",
                     es: "Conversa con conectores de archivos en vivo para la cosmología observacional. Pídele una cadena de verosimilitud BAO de DESI, un diagrama HR, un ajuste de distancia de SN Ia o una restricción de CMB comprimida de Planck — el asistente llama a las APIs de archivo reales, registra cada semilla y cada run id, y cada número que cita se rastrea hasta la herramienta que lo produjo." },
  ...
  "home.ed.3.body":{ en: "Gaia DR3, 2MASS PSC, PARSEC 3.9 — the archive version travels with every datum.",    zh: "Gaia DR3、2MASS PSC、PARSEC 3.9 —— 每一条数据都带着它来自的档案版本。",                           fr: "Gaia DR3, 2MASS PSC, PARSEC 3.9 — la version de l'archive voyage avec chaque donnée.",    es: "Gaia DR3, 2MASS PSC, PARSEC 3.9 — la versión del archivo viaja con cada dato." },
